...
class ArticleTagsIterator(object):
    """
    ...
    def __init__(self, conn, per_page = 100):
        ...
        self.conn = conn
        self.article_tags = ArticleTags(self.conn)
    ...
    def __len__(self):
        ...
        return self.article_tags.total or 0
    ...
    def __iter__(self):
        ...
        if not self.article_tags.pages:
            return

        for page in range(1, self.article_tags.pages + 1):
            if not self.article_tags.page == page:
                self.load_page(page = page)
            for article in self.article_tags:
                yield article
    ...
    def __getitem__(self, key):
        ...
        for list_id in requested_list_ids:
            ...
            for page_nr in range(1, self.article_tags.pages + 1):
                max_list_id = (page_nr * self.article_tags.per_page) - 1
            ...
            # Add requested article-tag to result
            list_id_in_page = list_id - ((page - 1) * self.article_tags.per_page)
            result.append(self.article_tags[list_id_in_page])
